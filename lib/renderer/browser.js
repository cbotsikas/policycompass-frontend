...
Service.checkResources = function(request) {
    ...
    request.browser.page.run(function(resolve){
        ...
        if (!request.browser.redirectURL){
            var locationHeader = renderer.headers.find(function(header){
                return header.name === 'location';
            });
            if (locationHeader) {
                request.browser.redirectURL = locationHeader.value;
            }
        }
        ...
        console.log(request.browser.resourcesPending, request.browser.redirectURL, JSON.stringify(renderer));
        ...
        if(request.browser.pageStatus !== null && request.browser.resourcesPending <= 0 && timeSinceLastResourceReceived < 5000) {
            clearInterval(request.browser.checkResourcesTimeout);
            request.browser.checkResourcesTimeout = null;
