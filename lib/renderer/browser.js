...
Service.onPageCreate = function(request) {
    ...
    request.browser.page.run(request.blockedResources || [], process.cwd().replace(/\\/g,'/'), function(blockedResources, rootPath){
        ...
        page.onResourceRequested = function(requestData, networkRequest) {
            ...
            page.renderer.resources[requestData.url]="";
            //var replaceIndex, newurl;
            //replaceIndex = requestData.url.search(/\/(?:app|bower_components)\/.*\.(?:js|css)/);
            //console.log('jaja',replaceIndex, requestData.url.indexOf('/app/'), requestData.url.indexOf('/bower_components/'));
            //
            //if(replaceIndex > 0) {
            //    console.log('will replace');
            //    console.log(rootPath, requestData.url.substring(replaceIndex));
            //    newurl = "file:///" + fs.absolute(rootPath + requestData.url.substring(replaceIndex));
            //    page.renderer.resources[requestData.url]="replaced with "+newurl;
            //    console.log(page.renderer.resources[requestData.url]);
            //    networkRequest.changeUrl(newurl);
            //}

            if(!requestData.aborted) {
                console.log('onResourceRequested', requestData.id, page.renderer.resourcesRequested, requestData.url);

            } else {
                page.renderer.resources[requestData.url]="aborted";
                console.log('aborted', requestData.url);
            }

        };
        page.onResourceReceived = function(response) {
            //console.log('onResourceReceived', response.id, response.stage, response.status, response.url);
            //console.log('Response (#' + response.id + ', stage "' + response.stage + '"): ' + JSON.stringify(response));
            page.renderer.resources[response.url]+=response.status;
            ...
            if (response.stage === 'end' && response.status) {
                if (response.id === 1) {
                    page.renderer.status = response.status;
                }
                page.renderer.resourcesReceived++;
                console.log('onResourceReceived', response.id,page.renderer.resourcesReceived, response.stage, response.status, response.url);
        ...
        page.onResourceTimeout = function(request) {
            ...
            page.renderer.resources[request.url]+="timeout "+request.errorCode;

            page.renderer.resourcesTimeout++;
            console.log('onResourceTimeout', request.id, page.renderer.resourcesTimeout);

        };
        page.onResourceError = function(resourceError) {
            page.renderer.resources[resourceError.url]+="error "+resourceError.errorCode;

            page.renderer.resourcesError++;
            console.log('onResourceError', resourceError.id, page.renderer.resourcesError, resourceError.errorCode, resourceError.url);
...
Service.checkResources = function(request) {
    ...
    request.browser.page.run(function(resolve){
        var page = this;
        //page.renderer.httpConnections = page.evaluate(function() {
        //    return window.httpConnections || {};
        //});
        //page.renderer.activeHttpConnections = page.renderer.httpConnections.active || 0;
        resolve(page.renderer);
    }).then(function(renderer){
        request.browser.resourcesPending = renderer.resourcesRequested - renderer.resourcesReceived - renderer.resourcesError - renderer.resourcesTimeout;// + renderer.activeHttpConnections;
        ...
        if (renderer.httpConnections && renderer.httpConnections.lastReceived){
            var ajaxLastReceived = new Date(renderer.httpConnections.lastReceived);
            if (ajaxLastReceived > request.browser.resourcesLastReceived) {
                request.browser.resourcesLastReceived = ajaxLastReceived;
            }
            console.log("last received: ajax:", renderer.httpConnections.lastReceived, "browser:", renderer.resourcesLastReceived);
        }

        console.log('resources', request.browser.resourcesPending, renderer.resourcesRequested, renderer.resourcesReceived, renderer.resourcesError, renderer.resourcesTimeout, renderer.activeHttpConnections)
        console.log('httpConnections', JSON.stringify(renderer.httpConnections));
        ...
        if(request.browser.pageStatus !== null && request.browser.resourcesPending <= 0 && timeSinceLastResourceReceived > 500) {
            clearInterval(request.browser.checkResourcesTimeout);
            request.browser.checkResourcesTimeout = null;
